...
function Fixed_Update () {
  ...
  ctx.fillText("Bombs"+": "+bombs, 810, 50);
  instructions();
  for(var i=0; i<cols; ++i){
    for(var j=0; j<rows; ++j){
      b.array[i][j].sketch();
  ...
  if(state==1){
    ...
    ctx.fillStyle = "rgb(0,0,0)";
    ctx.fillText("You lost!?", 865, 380);
  } else if(state==3){
    var img = document.getElementById("won");
    ctx.drawImage(img, 860, 270);
    ctx.font = "20px Impact";
    ctx.fillStyle = "rgb(0,0,0)";
    ctx.fillText("You won!", 865, 380);
  }
...
function instructions(){
  ...
  ctx.fillText("-Level: Hard", 810, 85);
  ctx.font = "26px Impact";
  ctx.fillText("-Instructions:", 810, 125);
  ctx.font = "18px Impact";
  ctx.fillText("-Left click: reveal", 810, 165);
  ctx.fillText("-Right click: flag", 810, 205);
  ctx.fillText("-r: restart", 810, 245);
}
